...
python train.py --kernel-type 5fold_b5_05ep --k-fold 5 --data-folder sampled_face/ --enet-type tf_efficientnet_b5_ns --n-epochs 5 --batch-size 32
python train.py --kernel-type 5fold_b5_35ep --k-fold 5 --data-folder sampled_face/ --enet-type tf_efficientnet_b5_ns --n-epochs 30 --batch-size 32
...
python train.py --kernel-type 25fold_b3_50ep --k-fold 25 --data-folder sampled_face/ --enet-type tf_efficientnet_b3_ns --n-epochs 50 --batch-size 16
...
def parse_args():
    ...
    parser.add_argument('--out-dim', type=int, default=1000)
...
def train_epoch(model, loader, optimizer):
    ...
    train_loss = []
    bar = tqdm(loader)
    for i, (data1,data2,data3, target) in enumerate(bar):
        ...
        if args.use_meta:
            ...
            data3, target = data3.to(device), target.to(device)
...
def val_epoch(model, loader, target_idx, is_ext=None, n_test=1, get_output=False):
    ...
    with torch.no_grad():
        for (data1, data2, target) in tqdm(loader):
            ...
            if args.use_meta:
                ...
                #logits = torch.zeros((args.batch_size)).to(device)
                #probs = torch.zeros((args.batch_size)).to(device)

                for I in range(n_test):
                    l1 = model(get_trans(data1, I))     #input1 = model(지문데이터 template )
                    l2 = model(get_trans(data2, I))     #input2 = model(지문데이터 probe )
            ...
            loss = nn.BCELoss()(logits.float(), target.float())
    ...
    #TARGETS = np.zeros(len(logits))
    if get_output:
        return LOGITS, PROBS
    else:
        # accuracy : 정확도
        acc = (LOGITS == TARGETS).mean() * 100.

        # AUC : ROC 아래 넓이
        #auc = accuracy_score((TARGETS == target_idx).astype(float), PROBS.astype(float))
...
def run(fold, df, meta_features, n_meta_features, transforms_train, transforms_val, target_idx):
    ...
    acc_max = 0.
    auc_no_ext_max = 0.
    model_file = os.path.join(args.model_dir, f'Greenbit_ori_{args.kernel_type}_best_fold{fold}.pth')
    model_file2 = os.path.join(args.model_dir, f'Greenbit_ori_{args.kernel_type}_best_no_ext_fold{fold}.pth')
    model_file3 = os.path.join(args.model_dir, f'Greenbit_ori_{args.kernel_type}_final_fold{fold}.pth')
    ...
    for epoch in range(1, args.n_epochs + 1):
        ...
        val_loss, acc = val_epoch(model, valid_loader, target_idx, is_ext=df_valid['is_ext'].values)
        ...
        print(content)
        with open(os.path.join(args.log_dir, f'log_Greenbit_ori_{args.kernel_type}.txt'), 'a') as appender:
