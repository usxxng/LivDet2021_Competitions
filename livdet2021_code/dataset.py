...
def get_df_stone(k_fold, data_dir, data_folder, out_dim = 1, use_meta = False, use_ext = False):
    ...
    df_train['fold'] = df_train['id'].map(faceid2fold)
...
class MMC_ClassificationDataset(Dataset):
    ...
    def __init__(self, csv, mode, meta_features, transform_anchor=None,transforms_positive = None,transforms_negative =None):
        ...
        self.transforms_negative = transforms_negative

        # robust test parameters and type
    ...
    def __getitem__(self, index):
        id_name = list(set(self.csv.id))
        random_number = ['1.png','2.png','3.png','4.png','5.png']
        row1 = self.csv.iloc[index]
        image1 = cv2.imread(row1.filepath) #anchor
        base = row1.filepath.split('_')

        row2 = base[0]+'_'+base[1]+'_'+base[2]+'_'+base[3]+'_'+base[4]+'_'+base[5]+'_'+random.choice(random_number)
        image2 = cv2.imread(row2) #positive

        value = random.choice(id_name)
        while(True):
            if value == int(base[2]):
                value = random.choice(id_name)
            else:
        ...
        row3 = base[0]+'_'+base[1]+'_'+str(value)+'_'+base[3]+'_'+base[4]+'_'+base[5]+'_'+base[6]
        image3 = cv2.imread(row3) #negative
        ...
        # 이미지 tranform 적용
        # image1
        if self.transform_anchor is not None:
            res = self.transform_anchor(image=image1)
            image1 = res['image'].astype(np.float32)
        else:
            image1 = image1.astype(np.float32)

        #image2
        if self.transforms_positive is not None:
            res = self.transforms_positive(image=image2)
            image2 = res['image'].astype(np.float32)
        else:
            image2 = image2.astype(np.float32)

        #image3
        if self.transforms_negative is not None:
            res = self.transforms_negative(image=image3)
            image3 = res['image'].astype(np.float32)
        else:
        ...
        if self.use_meta:
            data1 = (torch.tensor(image1).float(), torch.tensor(self.csv.iloc[index][self.meta_features]).float())
            data2 = (torch.tensor(image2).float(), torch.tensor(self.csv.iloc[index][self.meta_features]).float())
            data3 = (torch.tensor(image3).float(), torch.tensor(self.csv.iloc[index][self.meta_features]).float())
        else:
            data1 = torch.tensor(image1).float()
            data2 = torch.tensor(image2).float()
            data3 = torch.tensor(image3).float()


        if self.mode == 'valid':
            # Test 의 경우 정답을 모르기에 데이터만 리턴

            p = random.random() # 랜덤 생성
            if p > 0.5:
                target = 0
                return data1, data2, torch.tensor(target).long()
            else:
                target = 1
                return data1, data3, torch.tensor(target).long()
        else:
            # training 의 경우 CSV의 스톤여부를 타겟으로 보내줌
            smooth_target = self.csv.iloc[index].target
            return data1, data2, data3, torch.tensor(smooth_target).long()
...
def get_transforms(image_size):
    ...
    transforms_anchor = albumentations.Compose([
        ...
        #albumentations.Cutout(max_h_size=int(image_size * 0.375), max_w_size=int(image_size * 0.375), num_holes=1, p=0.4),
        albumentations.Normalize()
    ])

    transforms_positive = albumentations.Compose([
    ...
    transforms_negative = albumentations.Compose([
        #albumentations.HorizontalFlip(p=0.5),
    ...
    return transforms_anchor,transforms_positive,transforms_negative, transforms_val
